...
parser.add_argument("--includeZ", action="store_true",
                    help="Include Z events", default=False)
...
# z difference between truth PV and reconstructed PV
rdf = rdf.Define("PV_z_reco_diff", "PV_z - GenVtx_z")
...
rdf = rdf.Define("u_GEN_pt", "V_pt").Define(
    "u_GEN_x", "- V_pt * TMath::Cos(V_phi)").Define("u_GEN_y", "- V_pt * TMath::Sin(V_phi)")
...
if includeZ:
    ...
    rdf_Z_org = ROOT.ROOT.RDataFrame(
        "Events", "/eos/cms/store/group/cmst3/group/wmass/yofeng/NanoAOD/DYJetsToMuMu_H2ErratumFix_TuneCP5_13TeV-powhegMiNNLO-pythia8-photos/NanoV9MCPostVFP_testPVRobustDM/240226_075056/0000/NanoV9MCPostVFP_1*.root")
    rdf_Z_org1 = rdf_Z_org.Filter("nMuon > 1")
    rdf_Z_org1 = rdf_Z_org1.Define(
        "Muon_pass0", "Muon_pt[0] > 25.0 && abs(Muon_eta[0]) < 2.4 && Muon_pfRelIso04_all[0] < 0.15 && Muon_looseId[0]")
    rdf_Z_org1 = rdf_Z_org1.Define(
        "Muon_pass1", "Muon_pt[1] > 25.0 && abs(Muon_eta[1]) < 2.4 && Muon_pfRelIso04_all[1] < 0.15 && Muon_looseId[1]")

    rdf_Z_org2 = rdf_Z_org1.Filter("Muon_pass0 && Muon_pass1")
    rdf_Z = rdf_Z_org2

    rdf_Z = rdf_Z.Define(
        "V_pt", "TMath::Sqrt(Muon_pt[0] * Muon_pt[0] + Muon_pt[1] * Muon_pt[1] + 2 * Muon_pt[0] * Muon_pt[1] * TMath::Cos(Muon_phi[0] - Muon_phi[1]))")
    rdf_Z = rdf_Z.Define(
        "V_phi", "TMath::ATan2(Muon_pt[0] * TMath::Sin(Muon_phi[0]) + Muon_pt[1] * TMath::Sin(Muon_phi[1]), Muon_pt[0] * TMath::Cos(Muon_phi[0]) + Muon_pt[1] * TMath::Cos(Muon_phi[1]))")
    ...
    rdf_Z = rdf_Z.Define("u_GEN_pt", "V_pt").Define(
        "u_GEN_x", "- V_pt * TMath::Cos(V_phi)").Define("u_GEN_y", "- V_pt * TMath::Sin(V_phi)")

    h_pvIndex_Z = rdf_Z.Histo1D(
        ("pv_Index", "pv_Index", 11, -1.5, 10.5), "PVRobustIndex")
...
for met in ["PF", "PUPPI", "DeepMET"]:
    ...
    h_METs[met] = rdf.Histo1D(
        (hname, hname, 40, 0, 100), "{}MET_pt".format(met))
...
if not useGENMatching:
    rdf = rdf.Define("GoodPVEvent", "PVRobustIndex == 0").Define(
        "BadPVEvent", "PVRobustIndex != 0")
else:
    rdf = rdf.Define("GoodPVEvent", "abs(PV_z_reco_diff) < 0.02").Define(
        "BadPVEvent", "abs(PV_z_reco_diff) >= 0.02")
rdf_goodpv = rdf.Filter("GoodPVEvent")
rdf_badpv = rdf.Filter("BadPVEvent")


recoilanalyzer_goodpv = RecoilAnalyzer(
    rdf_goodpv, recoils, useRMS=useRMS, name="recoilanalyzer_goodpv")
...
recoilanalyzer_goodpv.prepareResolutions(
    'PV_npvsGood', xbins_nVtx, 400, -200, 200)

hresponses_goodpv = recoilanalyzer_goodpv.getResponses('u_GEN_pt')
hresols_paral_diff_goodpv, hresols_perp_goodpv = recoilanalyzer_goodpv.getResolutions(
    'u_GEN_pt')
hresponses_nVtx_goodpv = recoilanalyzer_goodpv.getResponses('PV_npvsGood')
hresols_paral_diff_VS_nVtx_goodpv, hresols_perp_VS_nVtx_goodpv = recoilanalyzer_goodpv.getResolutions(
    'PV_npvsGood')
...
for itype in recoils:
    ...
    values_responses_goodpv[itype] = hresponses_inclusive_goodpv[itype].GetBinContent(
        1)

recoilanalyzer_badpv = RecoilAnalyzer(
    rdf_badpv, recoils, useRMS=useRMS, name="recoilanalyzer_badpv")
...
recoilanalyzer_badpv.prepareResolutions(
    'PV_npvsGood', xbins_nVtx, 400, -200, 200)

hresponses_badpv = recoilanalyzer_badpv.getResponses('u_GEN_pt')
hresols_paral_diff_badpv, hresols_perp_badpv = recoilanalyzer_badpv.getResolutions(
    'u_GEN_pt')
hresponses_nVtx_badpv = recoilanalyzer_badpv.getResponses('PV_npvsGood')
hresols_paral_diff_VS_nVtx_badpv, hresols_perp_VS_nVtx_badpv = recoilanalyzer_badpv.getResolutions(
    'PV_npvsGood')
...
for itype in recoils:
    ...
    values_responses_badpv[itype] = hresponses_inclusive_badpv[itype].GetBinContent(
        1)
...
for pvs in ["GoodPV", "BadPV"]:
    ...
    h_W_qts[pvs] = tmp.Histo1D(
        (hname, hname, xbins_qT.size - 1, xbins_qT), "V_pt")
    h_reco_z_diffs[pvs] = tmp.Histo1D(
        ("h_reco_z_diff_{}".format(pvs), "h_reco_z_diff_{}".format(pvs), 100, -0.1, 0.1), "PV_z_reco_diff")
...
DrawHistos(h_pvIndices, h_pvIndices_labels, -1.5, 10.5, "Robust PV Index", 1e-5, 10.0, "A.U.",
           "W_pv_Index", donormalize=True, outdir=outdir, noLumi=noLumi, mycolors=[1, 2], MCOnly=MCOnly)

# make some kinematic distributions
DrawHistos(h_MTs.values(), [labels[itype] for itype in h_MTs.keys()], 0, 120, "m_{T} [GeV]", 0., 0.1, "A.U.", "MT_comp", drawashist=True, dology=False, legendPos=[
           0.20, 0.67, 0.38, 0.86], mycolors=[colors[itype] for itype in h_MTs.keys()], outdir=outdir, donormalize=True, MCOnly=MCOnly)

DrawHistos(h_METs.values(), [labels[itype] for itype in h_METs.keys()], 0, 100, "p^{miss}_{T} [GeV]", 0., 0.13, "A.U.", "MET_comp", drawashist=True, dology=False, legendPos=[
           0.62, 0.67, 0.80, 0.86], mycolors=[colors[itype] for itype in h_METs.keys()], outdir=outdir, donormalize=True, MCOnly=MCOnly)

DrawHistos(h_muon_pts.values(), ["PV Good", "PV Bad"], 0, 100, "#mu p_{T} [GeV]", 0., 0.15, "A.U.", "muon_pt_comp", drawashist=True, dology=False, legendPos=[
           0.65, 0.67, 0.90, 0.86], mycolors=[1]*2, linestyles=[1, 2], outdir=outdir, donormalize=True, MCOnly=MCOnly)

DrawHistos(h_muon_etas.values(), ["PV Good", "PV Bad"], -2.5, 2.5, "#mu #eta", 0., 0.05, "A.U.", "muon_eta_comp", drawashist=True,
           dology=False, legendPos=[0.20, 0.67, 0.38, 0.86], mycolors=[1]*2, linestyles=[1, 2], outdir=outdir, donormalize=True, MCOnly=MCOnly)

DrawHistos(h_W_qts.values(), ["PV Good", "PV Bad"], 0, 150, "W q_{T} [GeV]", 0., 0.3, "A.U.", "W_qt_comp", drawashist=True, dology=False, legendPos=[
           0.65, 0.67, 0.90, 0.86], mycolors=[1]*2, linestyles=[1, 2], outdir=outdir, donormalize=True, MCOnly=MCOnly)

DrawHistos(h_reco_z_diffs.values(), ["PV Good", "PV Bad"], -0.1, 0.1, "z_{reco} - z_{truth} [cm]", 0., 0.5, "A.U.", "reco_z_diff_comp", drawashist=True, dology=False, legendPos=[
    0.20, 0.67, 0.38, 0.86], mycolors=[1]*2, linestyles=[1, 2], outdir=outdir, donormalize=True, MCOnly=MCOnly, addOverflow=True, addUnderflow=True)
...
args = {
    ...
    "linestyles": [1] * len(hresponses_goodpv) + GetLineStyles(hresponses_badpv),
    "outdir": outdir,
    "noLumi": noLumi,
    "dology": False,
    "drawashist": True,
...
DrawHistos(list(hresponses_goodpv.values()) + list(hresponses_badpv.values()), [labels[itype] for itype in hresponses_goodpv.keys(
)], 0, qtmax, qtlabel, 0., 1.29, responselabel, "reco_recoil_response_" + suffix, legendPos=[0.50, 0.17, 0.80, 0.37], inPaper=True, **args)

DrawHistos(list(hresols_paral_diff_goodpv.values()) + list(hresols_paral_diff_badpv.values()), [labels[itype] for itype in hresols_paral_diff_goodpv.keys()], 0, qtmax, qtlabel, 0, 39.0, uparallabel, "reco_recoil_resol_paral", drawashist=True, dology=False, legendPos=[
           0.20, 0.73, 0.38, 0.92], mycolors=[colors[itype] for itype in hresols_paral_diff_goodpv.keys()]*2, noLumi=noLumi, outdir=outdir, linestyles=linestyles, MCOnly=MCOnly)

DrawHistos(list(hresols_perp_goodpv.values()) + list(hresols_perp_badpv.values()), [labels[itype] for itype in hresols_perp_goodpv.keys()], 0, qtmax, qtlabel, 0, 32.0, uperplabel, "reco_recoil_resol_perp",
           drawashist=True, dology=False, legendPos=[0.20, 0.73, 0.40, 0.92], mycolors=[colors[itype] for itype in hresols_perp_goodpv.keys()]*2, noLumi=noLumi, outdir=outdir, linestyles=linestyles, MCOnly=MCOnly)

DrawHistos(list(hresponses_nVtx_goodpv.values()) + list(hresponses_nVtx_badpv.values()), [labels[itype] for itype in hresponses_nVtx_goodpv.keys()], nvtxmin, nvtxmax, nvtxlabel, 0., 1.15, responselabel, "reco_recoil_response_VS_nVtx", drawashist=True, dology=False, legendPos=[
           0.45, 0.17, 0.88, 0.36], mycolors=[colors[itype] for itype in hresponses_goodpv.keys()]*2, noLumi=noLumi, outdir=outdir, linestyles=linestyles, MCOnly=MCOnly)

DrawHistos(list(hresols_paral_diff_VS_nVtx_goodpv.values()) + list(hresols_paral_diff_VS_nVtx_badpv.values()), [labels[itype] for itype in hresols_paral_diff_VS_nVtx_goodpv.keys()], nvtxmin, nvtxmax, nvtxlabel, 0, 60.0, uparallabel, "reco_recoil_resol_paral_VS_nVtx", drawashist=True, dology=False, legendPos=[
           0.20, 0.73, 0.38, 0.92], mycolors=[colors[itype] for itype in hresols_paral_diff_VS_nVtx_goodpv.keys()]*2, noLumi=noLumi, outdir=outdir, linestyles=linestyles, MCOnly=MCOnly)

DrawHistos(list(hresols_perp_VS_nVtx_goodpv.values()) + list(hresols_perp_VS_nVtx_badpv.values()), [labels[itype] for itype in hresols_perp_VS_nVtx_goodpv.keys()], nvtxmin, nvtxmax, nvtxlabel, 0, 60.0, uperplabel, "reco_recoil_resol_perp_VS_nVtx", drawashist=True, dology=False, legendPos=[
           0.20, 0.73, 0.40, 0.92], mycolors=[colors[itype] for itype in hresols_perp_VS_nVtx_goodpv.keys()]*2, noLumi=noLumi, outdir=outdir, linestyles=linestyles, MCOnly=MCOnly)


if applySc:
    extraToDraw = ROOT.TPaveText(0.60, 0.15, 0.90, 0.22, "NDC")
    extraToDraw.SetFillColorAlpha(0, 0)
    extraToDraw.SetBorderSize(0)
    extraToDraw.SetTextFont(42)
    extraToDraw.SetTextSize(0.04)
    ...
    DrawHistos(list(hresolsSc_paral_diff_goodpv.values()) + list(hresolsSc_paral_diff_badpv.values()), [labels[itype] for itype in hresolsSc_paral_diff_goodpv.keys(
    )], 0, qtmax, qtlabel, 0, 39.0, uparallabel, "reco_recoil_resol_paral_Scaled", legendPos=[0.30, 0.69, 0.65, 0.92], **args)

    DrawHistos(list(hresolsSc_perp_goodpv.values()) + list(hresolsSc_perp_badpv.values()), [labels[itype] for itype in hresolsSc_perp_goodpv.keys(
    )], 0, qtmax, qtlabel, 0, 32.0, uperplabel, "reco_recoil_resol_perp_Scaled", legendPos=[0.30, 0.69, 0.70, 0.92], **args)

    DrawHistos(list(hresolsSc_paral_diff_VS_nVtx_goodpv.values()) + list(hresolsSc_paral_diff_VS_nVtx_badpv.values()), [labels[itype] for itype in hresolsSc_paral_diff_VS_nVtx_goodpv.keys(
    )], nvtxmin, nvtxmax, nvtxlabel, 0, 59.0, uparallabel, "reco_recoil_resol_paral_Scaled_VS_nVtx_" + suffix, legendPos=[0.37, 0.58, 0.67, 0.78], inPaper=True, **args)

    DrawHistos(list(hresolsSc_perp_VS_nVtx_goodpv.values()) + list(hresolsSc_perp_VS_nVtx_badpv.values()), [labels[itype] for itype in hresolsSc_perp_VS_nVtx_goodpv.keys(
    )], nvtxmin, nvtxmax, nvtxlabel, 0, 59.0, uperplabel, "reco_recoil_resol_perp_Scaled_VS_nVtx_" + suffix, legendPos=[0.37, 0.58, 0.67, 0.78], inPaper=True, **args)
