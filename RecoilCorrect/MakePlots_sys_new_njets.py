...
def main():
    ...
    DYSamp    = Sample(input_dy,    xsec = 6077.22*1e3,      color=5,  reweightzpt = True, legend="DY", name="DY", bjetVeto=VetoB)
    ...
    if not dotest:
        ...
        WZ3LSamp  = Sample(input_WZ3L,  xsec = 5.26*1e3,         color=39, reweightzpt = False, legend="WZ3L",     name="WZ3L",  bjetVeto=VetoB)
        ZZ2LSamp  = Sample(input_ZZ2L,  xsec = 0.564*1e3,        color=37, reweightzpt = False, legend="ZZ2L",     name="ZZ2L",  bjetVeto=VetoB)
        ZZ4LSamp  = Sample(input_ZZ4L,  xsec = 1.212*1e3,        color=36, reweightzpt = False, legend="ZZ4L",     name="ZZ4L",  bjetVeto=VetoB) 
    ...
    if not dotest:
        sampMan = SampleManager(DataSamp, [DYSamp, WW2LSamp, WZ3LSamp, TTbarSamp, ZZ2LSamp, ZZ4LSamp])
    else:
        sampMan = SampleManager(DataSamp, [DYSamp, TTbarSamp])
    sampMan.groupMCs(["WW2L", "WZ3L", "ZZ2L", "ZZ4L"], "Dibosons", 38, "Dibosons")
    ...
    sampMan.cacheDraw("met_phi", "histo_zjets_pfmet_phi", 30, phimin, phimax, DrawConfig(xmin=phimin, xmax=phimax, xlabel='PF #phi'))
    sampMan.cacheDraw("puppimet_pt", "histo_zjets_puppimet_pt", met_pt_bins, DrawConfig(xmin=0, xmax=200, xlabel='PUPPI MET [GeV]'))
    sampMan.cacheDraw("puppimet_phi", "histo_zjets_puppimet_phi", 30, phimin, phimax, DrawConfig(xmin=phimin, xmax=phimax, xlabel='PUPPI #phi'))

    # uncorrected deepmet
    sampMan.cacheDraw("deepmet_pt", "histo_zjets_deepmet_pt", met_pt_bins, DrawConfig(xmin=0, xmax=200, xlabel='Deep MET [GeV]'))
    sampMan.cacheDraw("deepmet_phi", "histo_zjets_deepmet_phi", 30, phimin, phimax, DrawConfig(xmin=phimin, xmax=phimax, xlabel='Deep MET #phi'))
    ...
    def compRatios(hnames, colors, linestyles, legends, outputtag):
        histos_met =  [sampMan.hratios["histo_zjets_deepmet_pt_{}".format(hname)].values()[0] for hname in hnames]
        histos_u_pt = [sampMan.hratios["histo_zjets_u_pt_{}".format(hname)      ].values()[0] for hname in hnames]
        histos_u1   = [sampMan.hratios["histo_zjets_u1_{}".format(hname)        ].values()[0] for hname in hnames]
    ...
    if doDump:
        ...
        for branchName in ["zpt", "deepmet_pt", "u1", "u2", "u1_corr_njets_3Gauss", "u2_corr_njets_3Gauss", "u_pt_corr_njets_3Gauss", "deepmet_pt_corr_njets_3Gauss","weight", "weight_WoVpt", "vtx_n", "u1_PF", "u2_PF", "u1_PUPPI", "u2_PUPPI", "jet_CSVLoose_n"]:
        ...
        branchList = ROOT.vector('string')()
        for branchName in ["zpt", "deepmet_pt", "u1", "u2", "weight", "weight_WoVpt", "vtx_n", "u1_PF", "u2_PF", "u1_PUPPI", "u2_PUPPI", "jet_CSVLoose_n"]:
            branchList.push_back(branchName)
