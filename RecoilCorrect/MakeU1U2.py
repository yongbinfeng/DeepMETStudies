...
def makeU1U2(cat=0):
    ...
    if isData:
        samp = Sample(input_data, isMC=False, legend="Data",
        ...
        samp = Sample(input_dy,    xsec=0,  color=5,  reweightzpt=False,
        ...
        if reweightzpt:
            ...
            h_weighted = samp.rdf.Histo1D(
                ("h_weighted", "h_weighted", 2, 0, 2.0), "dumbVal", "weight_bare")
            h_woweight = samp.rdf.Histo1D(
        ...
        samp = Sample(input_ttbar, xsec=0,  color=46, reweightzpt=False,
    ...
    def prepareU1U2(rdf, postfix="", extra_weight="1.0", njetbins=njetbins, ptbins=ptbins):
        ...
        for ijet in range(njetbins.size-1):
            ...
            for ipt in range(ptbins.size-1):
                ...
                rdf = rdf.Define(wstring, "(jet_n>={} && jet_n<={}) * (Z_pt>={} && Z_pt<{} ) * {} * norm * {}".format(
                    njetmin, njetmax, ptmin, ptmax, weightstr, extra_weight))
                ...
                histos_u1[(njetmin, njetmax)][(ptmin, ptmax)] = rdf.Histo1D(
                    (hname_u1, hname_u1, 350, -350.0-0.5*(ptmax-ptmin), 350.0+0.5*(ptmax-ptmin)),  "u1diff",  wstring)
                hname_u2 = "hist_uperp_{}".format(wstring)
                histos_u2[(njetmin, njetmax)][(ptmin, ptmax)] = rdf.Histo1D(
                    (hname_u2, hname_u2, 300, -150.0,       150.0),       "u2",  wstring)
    ...
    for samp in samples:
        ...
        for wname, wstring in weights.items():
            ...
            histos_u1[wname], histos_u2[wname] = prepareU1U2(samp.rdf, postfix="{}_{}".format(
                samp.name, wname), extra_weight=wname, njetbins=njetbins, ptbins=ptbins)

            histos_u1_njetInclusive[wname], histos_u2_njetInclusive[wname] = prepareU1U2(samp.rdf, postfix="{}_{}".format(
                samp.name, wname), extra_weight=wname, njetbins=njetbinsInclusive, ptbins=ptbins)
        ...
        for wname, wstring in weights.items():
            ...
            for ijetbin in list(histos_u1[wname].keys()):
                for iptbin in list(histos_u1[wname][ijetbin].keys()):
                    histos_u1[wname][ijetbin][iptbin].GetValue(
                    ).SetDirectory(ofile)
                    histos_u1[wname][ijetbin][iptbin].GetValue().Write()
                    histos_u2[wname][ijetbin][iptbin].GetValue(
                    ).SetDirectory(ofile)
                    histos_u2[wname][ijetbin][iptbin].GetValue().Write()
            ofile.Close()

            ofilename = "results/U1U2/histos_u1u2_{}_njetsInclusive_pt_{}.root".format(
                samp.name, wname)
            print("Write to output file ", ofilename)
            ofile = ROOT.TFile(ofilename, "recreate")
            for ijetbin in list(histos_u1_njetInclusive[wname].keys()):
                for iptbin in list(histos_u1_njetInclusive[wname][ijetbin].keys()):
                    histos_u1_njetInclusive[wname][ijetbin][iptbin].GetValue(
                    ).SetDirectory(ofile)
                    histos_u1_njetInclusive[wname][ijetbin][iptbin].GetValue(
                    ).Write()
                    histos_u2_njetInclusive[wname][ijetbin][iptbin].GetValue(
                    ).SetDirectory(ofile)
                    histos_u2_njetInclusive[wname][ijetbin][iptbin].GetValue(
                    ).Write()
            ofile.Close()
